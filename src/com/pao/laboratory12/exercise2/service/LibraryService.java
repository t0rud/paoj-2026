...
// Importam conexiunea din exercitiul tau 1!
        ...
        try {
            ...
            try (PreparedStatement checkPs = conn.prepareStatement(checkSql)) {
                ...
                try (ResultSet rs = checkPs.executeQuery()) {
                    ...
                    if (rs.getInt("available") == 0) throw new SQLException("Cartea cu id=" + bookId + " nu este disponibila.");
                }
            }

            // 2. Insereaza imprumutul
            ...
            // 3. Marcheaza cartea ca indisponibila
            ...
            conn.setAutoCommit(true); // Restauram mereu starea initiala in finally
        }
    }
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                results.add(String.format("Loan#%d | '%s' -> %s | din %s",
                        rs.getLong("loan_id"), rs.getString("book_title"),
                        rs.getString("reader_name"), rs.getString("loan_date")));
            }
        }
        return results;
    }
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                results.add(String.format("'%s' de %s — %d imprumuturi",
                        rs.getString("book_title"), rs.getString("author_name"), rs.getLong("borrow_count")));
            }
        }
        return results;
    }
